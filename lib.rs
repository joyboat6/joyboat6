...
contract! {
    ...
    impl Erc20 {
        ...
        pub(external) fn allowance(&self, owner: AccountId, spender: AccountId) -> Balance {
            ...
            env.println(&format!("Erc20::allowance(owner = {:?}{:?}) = {:?}", owner, spender,allowance));
            allowance
        }
        ...
        pub(external) fn approve(&mut self, spender: AccountId, value: Balance) -> bool {
            ...
            self.allowances.insert((env.caller(),spender), value);
            env.emit(Approve { 
                a: Some(env.caller()),
                b: Some(spender),
                value: value
        ...
        pub(external) fn transfer_from(&mut self, from: AccountId, to: AccountId, value: Balance) -> bool {
            ...
            if allowance > value{
                let nowallowance = allowance - value;
                self.allowances.insert((from,env.caller()), nowallowance);
                return self.transfer_impl(env,from,to,value)
            }
            false
        }
    }
    ...
    impl Erc20 {
        ...
        fn allowance_or_zero(&self, owner: &AccountId, spender: &AccountId) -> Balance {
            ...
            *self.allowances.get(&key).unwrap_or(&0)
        }
...
mod tests {
    ...
    fn events_work() {
        ...
        // No events to start
        env::test::set_caller::<Types>(alice);
        assert_eq!(env::test::emitted_events::<Types>().count(), 4);
        ...
        assert_eq!(env::test::emitted_events::<Types>().count(), 6);
        // Event should be emitted for approvals
        assert_eq!(erc20.approve(bob, 20), true);
        assert_eq!(env::test::emitted_events::<Types>().count(), 7);
    }
}
